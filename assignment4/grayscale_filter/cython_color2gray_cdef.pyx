...
def cython_color2gray_cdef(imagefile, outfile=None):
    """
    ...
    bgr_image = bgr_image.astype("float64")
    grayscale_image = cython_grayscale_filter(bgr_image).astype("uint8")
...
cdef cython_grayscale_filter(np.ndarray[np.double_t, ndim=3] bgr_image):

    cdef int H = bgr_image.shape[0]
    cdef int W = bgr_image.shape[1]

    cdef np.ndarray[np.double_t, ndim=2] grayscale_image = np.empty((H, W))
